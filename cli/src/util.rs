...
pub fn menu_cmd() -> Option<Command> {
    env::var_os("FREEPASS_MENU").or(env::var_os("MENU")).map(
        |s| {
...
pub fn read_password_askpass(mut command: Command) -> SecStr {
    let process = command.stdout(Stdio::piped()).spawn().expect(
        "Couldn't spawn askpass program",
    );
...
pub fn read_text_console(prompt: &str) -> Option<String> {
    let mut tty = fs::OpenOptions::new()
        ...
        .write(true)
        .open("/dev/tty")
        .expect("Couldn't open terminal device /dev/tty");
...
pub fn read_text_asktext(mut command: Command, prompt: &str) -> Option<String> {
    match command
              .env("FREEPASS_MODE", "TEXT")
              .env("FREEPASS_PROMPT", prompt)
              .output() {
        Ok(ref output) if output.status.success() => Some(String::from_utf8_lossy(&output.stdout).replace("\n", "")),
        _ => None,
    }
}
