...
impl ImportVault {
    pub fn new() -> ImportVault {
        ImportVault { entries: BTreeMap::new() }
    }
}
    ...
    for node in &db.root {
        match node {
            ...
            Node::Entry(kentry) => {
                ...
                vault.entries.insert(kentry.get_title().unwrap_or("??? Untitled imported entry").to_owned(), (entry, EntryMetadata::new()));
            }
        }
    }
    Ok(vault)
